...
def _extract(max_trajectories: int) -> List[List[Dict[str, float]]]:
    """Return per-trajectory lists of governed assistant steps (token counts only)."""
    if CACHE.exists():
        import pandas as pd
    ...
    try:
        ...
        pd.DataFrame(rows, columns=["traj", "turn", "prompt_tokens", "completion_tokens"]
                     ).to_parquet(CACHE)
...
def _paired_ci(base: np.ndarray, treat: np.ndarray, boot: int, rng) -> Dict[str, float]:
    ...
    reds.sort()
    return {"point": point, "lo": float(reds[int(0.025 * boot)]), "hi": float(reds[int(0.975 * boot)])}
...
def main(argv: Any = None) -> int:
    ...
    trajs = _extract(args.max_trajectories)
    ...
    for name, feats in CONDITIONS:
        ...
        for j, tr in enumerate(trajs):
            tk, us, cr, tp = _traj_metrics(tr, feats, bool(route_simple[j]), cost_model,
                                           carbon_fixed, scope_cap, max_loops)
    ...
    for name, _ in CONDITIONS:
        entry: Dict[str, Any] = {"breaker_trips": trips[name],
                                 "breaker_trip_rate": trips[name] / len(trajs)}
        if name != "baseline":
            entry["reduction_ci"] = {m: _paired_ci(base[m], per[name][m], args.bootstrap, bootrng)
                                     for m in ("tokens", "usd", "carbon")}
        conditions[name] = entry

    data = {
        "dataset": "nebius/SWE-rebench-openhands-trajectories (CC-BY-4.0)",
        "n_trajectories": len(trajs), "median_depth": median_depth,
        "max_depth": int(max(depths)), "median_prompt_tokens": float(np.median(all_prompts)),
        "scope_cap": scope_cap, "breaker_max_loops": max_loops,
        "conditions": conditions, "snowball_fit": snowball_fit(trajs),
    }
    out = Path(args.out)
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(data, indent=2), encoding="utf-8")

    print(f"wrote {out}: {len(trajs)} trajectories, median depth {median_depth:.0f}, "
          f"max {max(depths)}, scope_cap {scope_cap}, breaker max_loops {max_loops}")
    for name, _ in CONDITIONS[1:]:
        r = conditions[name]["reduction_ci"]
        print(f"  {name:<14} tok {r['tokens']['point']:5.1f}% usd {r['usd']['point']:5.1f}% "
              f"carbon {r['carbon']['point']:5.1f}%")
    print(f"  +full breaker trip rate: {conditions['+full']['breaker_trip_rate']*100:.1f}% "
          f"of trajectories")
    sf = data["snowball_fit"]
    print(f"  snowball: median c2={sf['median_c2']:.1f} vs p/2={sf['theoretical_c2_p_over_2']:.1f}, "
